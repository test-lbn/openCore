  ...
  callWithJQuery(function($) {
        ...
        return function(arg) {
          ...
          return function(data, rowKey, colKey) {
            return {
              ...
              value: function() {
                return (this.sumNum / this.sumDenom) * 100;
              },
              format: formatter,
              numInputs: (num != null) && (denom != null) ? 0 : 2
            };
    ...
    aggregators = (function(tpl) {
      return {
        ...
        "Sumar": tpl.sum(usFmt),
        "Sumar Enteros": tpl.sum(usFmtInt),
        ...
        "Suma Sobre Suma * 100": tpl.irc(usFmt),
        "80% de limite superior": tpl.sumOverSumBound80(true, usFmt),
        "80% de limite inferior": tpl.sumOverSumBound80(false, usFmt),
        "Suma como fraccion del total": tpl.fractionOf(tpl.sum(), "total", usFmtPct),
        "Suma como fraccion de filas": tpl.fractionOf(tpl.sum(), "row", usFmtPct),
        "Suma como fraccion de columnas": tpl.fractionOf(tpl.sum(), "col", usFmtPct),
        "Cuenta como Fraccion de Total": tpl.fractionOf(tpl.count(), "total", usFmtPct),
        "Cuenta como fraccion de filas": tpl.fractionOf(tpl.count(), "row", usFmtPct),
        "Cuente como Fraccion de Columnas": tpl.fractionOf(tpl.count(), "col", usFmtPct)
      };
    })(aggregatorTemplates);
    renderers = {
    ...
    PivotData = (function() {
      ...
      PivotData.prototype.getAggregatorD = function(rowKey, colKey,porce) {
        var agg, flatColKey, flatRowKey;
    ...
    pivotTableRenderer = function(pivotData, opts) {
      ...
      th = document.createElement("th");
      th.className = "pvtTotalLabel";
      th.innerHTML = "Porcentaje";
      th.setAttribute("colspan", rowAttrs.length + (colAttrs.length === 0 ? 0 : 1));
      tr.appendChild(th);


      for (j in colKeysD) {
        if (!hasProp.call(colKeysD, j)) continue;
        colKey = colKeysD[j];
        
        totalAggregator = pivotData.getAggregatorD([], colKey,0);
        val = totalAggregator.value();
        td = document.createElement("td");
        td.className = "pvtTotal2 colTotal";

      

         
        valPorce = (val/valroral*100).toFixed(2)
        
      
        

        td.textContent = valPorce + "%";
        td.setAttribute("data-value", 2);
        if (getClickHandler != null) {
          td.onclick = getClickHandler(7, [], 8);
        }
        td.setAttribute("data-for", "col" + 9);
        tr.appendChild(td); 
      }
